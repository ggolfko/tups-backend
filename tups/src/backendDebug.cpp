...
int main()
{
    ...
    try
    {
        ...
        bClient.setTube(config.TUBEDEBUG);
...
#ifdef DEBUG
            cout << "Debug id: " << bClient.getId(job) << endl;
#endif
...
void workerTask(BeanstalkClient &bClient, Job &job, CONFIG config)
{
    ...
    string debugId = bClient.getId(job);

    try
    {
        Debug objDebug(atoi(debugId.c_str()), strWorkDIR, config);
        objDebug.run();
    }
